...
?>
			...
			if (isset($_POST['gname']) == false || isset($_POST['sname']) == false || isset($_POST['sex']) == false || isset($_POST['dob']) == false || isset($_POST['phone']) == false || isset($_POST['address']) == false){
				...
				$existing = singleSQL("SELECT Firstname, Lastname, dob FROM 201_patients WHERE Firstname='" . $_POST['gname'] . "' AND Lastname='" . $_POST['sname'] . "' AND dob='" . $_POST['dob'] . "'",$mysqli);
				if($existing!=0){
					$message = "Patient already exists.";
				} else{
					
					$newID = singleSQL("SELECT PatientNo FROM 201_patients ORDER BY PatientNo DESC LIMIT 1",$mysqli) + 1;
					
					$formattedDOB = date("d-m-Y", strtotime($_POST['dob']));
					
					$patientinfo = runSQL("INSERT INTO 201_patients (PatientNo, Firstname, Lastname, sex, email, dob, phone, address, medicare, insurance) VALUES ($newID,'" . $_POST['gname']."', '".$_POST['sname']."', '".$_POST['sex']."', '".$_POST['email']."', '".$formattedDOB."', '".$_POST['medicare']."', '".$_POST['phone']."', '".$_POST['address']."', '".$_POST['insurance']."');",$mysqli);
