		...
		<!-- CONTENT START -->
			...
			if(isset($_POST['password']) && isset($_POST['npassword'])){
				...
				if(($id == $_SESSION['User'])||checkFilePermission(__FILE__, $mysqli)){
					...
					if ($pw == singleSQL("SELECT Password FROM 201_users WHERE Username='" . $id ."'",$mysqli) || checkFilePermission(__FILE__, $mysqli)){
						echo '<center>';
						if(runSQL("UPDATE 201_users SET Password='$npw' WHERE Username='$id'", $mysqli)){
			...
			if(checkFilePermission(__FILE__, $mysqli) && isset($_POST['Username']) && isset($_POST['Firstname']) && isset($_POST['Lastname']) && isset($_POST['type']) && isset($_POST['AccessCode']) && isset($_POST['Room'])){
				...
				$up1 = runSQL("UPDATE 201_users SET Username='$un',AccessCode='$ac' WHERE Username='$id'", $mysqli);
				if ($up1){
					$sql = "UPDATE 201_staff SET Lastname='$ln',Firstname='$fn',Stafftype='$st',Username='$un', Room='$add' WHERE Username='$id'";
				...
				$rows = singleRowSQL("SELECT s.Username, Firstname, Lastname, Stafftype, Accesscode, Room FROM 201_users u join 201_staff s on u.Username = s.Username WHERE s.Username ='".$target."'" ,$mysqli);
			?>
