    ...
    */
    ...
    class PorterStemmer
    {
        ...
        */
	    ...
	    /**
        * Stems a word. Allows step disabling
        *
        * @param  string $word Word to stem
        * @return string       Stemmed word
        */
        public static function DStem($word,$ab,$c,$d,$e,$f,$g)
        {
            if (strlen($word) <= 2) {
                return $word;
            }
		...
		/**
        * French Stems a word with some basic rules. Allows step disabling
        *
        * @param  string $word Word to stem
        * @return string       Stemmed word
        */
		public static function FStem($word,$a,$b)
        {
            if (strlen($word) <= 2) {
                return $word;
            }
			...
			if ($a){
				$word = self::fstep1($word);
			}
			if ($b){
				$word = self::fstep2($word);
			}
        ...
        * @param string $word Word to stem
        */
        private static function fstep1($word)
        {
			if(substr($word, -2, 1) === 'n'){
				if(preg_match('/[^Mm]ent/',$word)){
					$word = preg_replace('/ent$/', '', $word);
				}
            }
            return $word;
        }
        ...
        * @param string $word Word to stem
        */
		private static function fstep2($word)
        {
			if(substr($word, -1) === 's' && strlen($word) != 4){
				$word = preg_replace('/([aeiou]{3}|[aeiou][aeou]|[^aeiou][aeiou]|[^aeiou])s$/', '$1', $word);
            }
            return $word;
        }
    }
