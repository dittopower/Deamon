...
?>
	...
	<!-- CONTENT START -->
		...
		if(isset($_GET['id']) && isset($_POST['PatientNo']) && isset($_POST['Firstname']) && isset($_POST['Lastname']) && isset($_POST['sex']) && isset($_POST['dob']) && isset($_POST['address']) && isset($_POST['phone']) && isset($_POST['email']) && isset($_POST['medicare']) && isset($_POST['insurance']) && isset($_POST['Notes'])){
			...
			$sql = "UPDATE patients SET Lastname='$ln',Firstname='$fn',Sex='$gender',Notes='$not', dob='$dateofbirth', address='$add',phone='$ph', email='$em', medicare='$med', insurance='$insur' WHERE PatientNo=$id";
		...
		if(isset($_POST['ward']) && isset($_POST['doctor']) && isset($_POST['id'])){
			...
			$numUsed = singleSQL("SELECT COUNT(*) FROM patientlocation WHERE WardID='$w'",$mysqli);

			

			$numAvail = singleSQL("SELECT NoOfBeds FROM wards WHERE WardID='$w'",$mysqli);

			

			$wardd = multiSQL("SELECT PatientNo FROM patientlocation WHERE PatientNo=$id",$mysqli);
			...
			if($wardd->num_rows === 0){
				...
				if(($numAvail - $numUsed) > 0){					
					...
					if(runSQL("INSERT INTO patientlocation (PatientNo, DoctorID, BedNo, WardId) VALUES($id, $d, 0, '$w')",$mysqli)){
						...
						$numward = singleSQL("SELECT count(billID)+1 as num FROM patientfinancial WHERE billID like 'ward%'",$mysqli);

						$fee = runSQL("INSERT INTO patientfinancial (billID, PatientID, OperationID, MoneyOwed) VALUES('ward-$numward', $id,'$opID','$cost')", $mysqli);
			...
			$rows = singleRowSQL("SELECT PatientNo, Notes, Firstname, Lastname, sex, email, dob, phone, address, medicare, insurance FROM patients WHERE PatientNo=". $_GET['id'] ,$mysqli);
		...
		<?php
			...
			<?php 
				...
				if($t->num_rows !== 0){
					...
					$tot = singleSQL("SELECT sum(MoneyOwed) as total FROM patientfinancial where PatientID = '$id'",$mysqli);
			...
			<?php 
				...
				$t = multiSQL("SELECT XrayFileID, Xray,XrayDate, StaffID FROM xray WHERE PatientID='$id' ORDER BY XrayDate DESC",$mysqli);
