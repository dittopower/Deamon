...
if(checkFilePermission(__FILE__, $mysqli)){
	...
	if(isset($_POST['Prescription']) && isset($_POST['Amount']) && isset($_POST['Price']) && $_POST['Prescription']!=""){
		...
		$cost = singleSQL("SELECT OperationCost FROM prices where OperationID = '$opID' ORDER BY OperationCost DESC LIMIT 1", $mysqli) + $price;

		$sp = "INSERT INTO patientfinancial (billID, PatientID, OperationID, MoneyOwed)	VALUES('Script-$newID',$id,'$opID','$price')";
