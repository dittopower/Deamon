...
?>
	...
	<?php
		...
		$t = singleRowSQL("SELECT Firstname, Lastname FROM staff WHERE Username='$user'",$mysqli);
	...
	<?php
		...
		$s = multiSQL("SELECT TheaterID, CONCAT_WS(' ',Firstname,Lastname) as Patient, StartTime FROM bookings join patients on Patient = patientNo WHERE BookerID='$staffid'",$mysqli);
		...
		$s = multiSQL("SELECT PatientNo, WardId, EnterTime FROM patientlocation WHERE DoctorID='$staffid'",$mysqli);
