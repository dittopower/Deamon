...
?>
			...
			if(isset($_POST['test'])){
				...
				foreach($files as $entry){
					...
					$u = singleSQL("SELECT FileName FROM filepermissions WHERE FileName='$entry'", $mysqli);

					if($u == null){

						$sql = "INSERT INTO filepermissions (FileName, Access) VALUES('$entry','$value')";

					}

					else{

						$sql = "UPDATE filepermissions SET Access='$value' WHERE FileName='$entry'";
			...
			foreach($files as $entry){
				...
				$y = singleSQL("SELECT Access FROM filepermissions WHERE FileName='$entry'",$mysqli);
