...
?>
	...
	<?php
		...
		if(isset($_POST["theater"]) && isset($_POST["date"])){
			...
			$room=singleSQL("SELECT RoomType FROM theaters WHERE TheaterID='$th'",$mysqli);

			$mowed = singleSQL("SELECT OperationCost FROM prices WHERE Operation='$room'",$mysqli);

			$opID = singleSQL("SELECT OperationID FROM prices WHERE Operation='$room'",$mysqli);

			$billID = $opID ."-". singleSQL("SELECT COUNT(billID) FROM patientfinancial WHERE billID LIKE '%$opID%'",$mysqli);
