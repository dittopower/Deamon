...
?>
	...
	<?php if(isset($_POST['firstName']) && isset($_POST['type'])){
		...
		$exist = singleSQL("SELECT Username FROM users WHERE Username='$username'", $mysqli);	
		...
		if($p1 === $p2 && $exist == null){
			...
			$sql1 = runSQL("INSERT INTO users (Username, Password, AccessCode) VALUES('$username','$p1','$accessCode')", $mysqli);

			

			if($sql1){

				$staffid = singleSQL('SELECT StaffID FROM staff ORDER BY StaffID DESC LIMIT 1', $mysqli) + 1;

				

				$sql2 = runSQL("INSERT INTO staff (Username, staffID, Stafftype, Lastname, Firstname) VALUES('$username',$staffid,'$type','$surname','$firstname')", $mysqli);
