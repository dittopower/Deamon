<?php 
	...
	$rows = singleRowSQL("SELECT PatientNo, Notes, Firstname, Lastname, sex, email, dob, phone, address, medicare, insurance FROM patients WHERE PatientNo=$id" ,$mysqli);

	$xr = multiSQL("SELECT XrayFileID, Xray,XrayDate, StaffID FROM xray WHERE PatientID='$id' ORDER BY XrayDate DESC",$mysqli);
		...
		<?php 
			...
			<?php if(isset($_POST['Prescription']) && isset($_POST['Amount']) && isset($_POST['Price']) && $_POST['Prescription']!=""){
					...
					$newID=singleSQL("SELECT PrescriptionID FROM prescriptions ORDER BY PrescriptionID DESC LIMIT 1",$mysqli)+1;

					

					$ss = "INSERT INTO prescriptions (PrescriptionID, PatientID, Price, PrescriptionName, Amount)
				...
				$s = multiSQL("SELECT PrescriptionID, PrescriptionName, Amount, Price FROM prescriptions WHERE PatientID=$id ORDER BY PrescriptionID DESC",$mysqli);
