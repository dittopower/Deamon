			...
			<!-- CONTENT START -->
				...
				<?php
					...
					if(isset($_GET['search']) && $_GET['search'] != ""){	
						...
						$r = multiSQL("SELECT p.PatientNo, p.Firstname, p.Lastname, sex, email, phone, s.Lastname as Doctor, WardID FROM patients p left join patientlocation l on p.patientNo = l.patientNo left join staff s on s.StaffID = l.DoctorID WHERE p.Firstname LIKE '%" . $_GET['search'] . "%' OR p.Lastname LIKE '%" . $_GET['search'] . "%' OR p.PatientNo LIKE '%" . $_GET['search'] . "%' ORDER BY Doctor,WardID,p.Lastname,p.Firstname ASC",$mysqli);
						...
						$r = multiSQL("SELECT p.PatientNo, p.Firstname, p.Lastname, sex, email, phone, s.Lastname as Doctor, WardID FROM patients p left join patientlocation l on p.patientNo = l.patientNo left join staff s on s.StaffID = l.DoctorID ORDER BY Doctor,WardID,p.Lastname,p.Firstname ASC",$mysqli);
