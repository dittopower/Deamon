	...
	<div id='inner'>
		...
		*/
		if(isset($_GET['createUser']) && isset($_POST['newUser']) && isset($_POST['newPass1']) && isset($_POST['newPass2']) && isset($_POST['newFname']) && isset($_POST['newLname']) && isset($_POST['newEmail'])){
			...
			if(isset($_POST['newSub'])){ $nsub = 1; }
			else{ $nsub = 0;}
			
			if(!strpos($nuser,' ')){
				if($npass == $npass2){
					if(filter_var($nemail, FILTER_VALIDATE_EMAIL)){
						$sql="INSERT INTO members(username, password, first_name, last_name, email, get_newsletter) VALUES ('$nuser', '$npass', '$nfname', '$nlname', '$nemail', $nsub)";
			...
			<form method='post' action='userAuth.php?createUser=true'>
				...
				<label>Subscribe to Newsletter: </label><input type='checkbox' name='newSub'><br><br>
		...
		<?php } else if(isset($_POST['username']) && isset($_POST['password'])){
			...
			$dbPassword = singleSQL("SELECT password FROM members WHERE username='$user'", $mysqli);
