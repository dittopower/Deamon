			...
			<!-- CONTENT START -->
				...
				<?php
					if(isset($_GET['search']) && $_GET['search'] != ""){	
						$r = multiSQL("SELECT u.Username, Stafftype, AccessCode, Firstname, Lastname, Room FROM 201_users u join 201_staff s on u.Username = s.Username WHERE Firstname LIKE '%" . $_GET['search'] . "%' OR Lastname LIKE '%" . $_GET['search'] . "%' OR u.Username LIKE '%" . $_GET['search'] . "%' OR Stafftype LIKE '%" . $_GET['search'] . "%' OR Room LIKE '%" . $_GET['search'] . "%' OR AccessCode LIKE '%" . $_GET['search'] . "%'ORDER BY AccessCode, Lastname, Firstname ASC",$mysqli);
						if($r->num_rows === 0){
						...
						$r = multiSQL("SELECT u.Username, Stafftype, AccessCode, Firstname, Lastname, Room FROM 201_users u join 201_staff s WHERE u.Username = s.Username ORDER BY Lastname, Firstname ASC",$mysqli);
