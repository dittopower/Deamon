...
?>
	...
	<center>
		<?php if(isset($_FILES["xrayImage"]["name"]) && isset($_POST["patientID"])){/* etc */
			...
			$newID = singleSQL('SELECT XrayFileId FROM 201_xray ORDER BY XrayFileId DESC LIMIT 1', $mysqli) + 1;
			...
			$fee = runSQL("INSERT INTO 201_patientfinancial (billID, PatientID, OperationID, MoneyOwed) VALUES('xray-$newID', $patientID,'$opID','$cost')", $mysqli);
			
			$sql="INSERT INTO 201_xray (XrayFileId, Xray, PatientID, StaffID, XrayDate) VALUES($newID,'$fileName',$patientID,$StaffID,CURRENT_TIMESTAMP())";
